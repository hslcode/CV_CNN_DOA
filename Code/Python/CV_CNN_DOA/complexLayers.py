...
import torch
from torch.nn import Module, Parameter, init,SELU
from torch.nn import Conv2d, Linear, BatchNorm1d, BatchNorm2d
from torch.nn import ConvTranspose2d
from complexFunctions import complex_relu, complex_max_pool2d, complex_avg_pool2d,zReLU
...
class CSELU(Module):
    def __init__(self, alpha=1.67326, lamb=1.0507):
        super(CSELU, self).__init__()
        self.alpha = alpha
        self.lamb = lamb

    def forward(self, x):
        return self.lamb *(torch.max(torch.zeros_like(x), x) +torch.min(torch.zeros_like(x), self.alpha * (torch.exp(x) - 1)))
class C_CSELU(Module):
    def __init__(self):
        super().__init__()
        self.CSELU = SELU()
    def forward(self, x):
        return torch.complex(self.CSELU(x.real),self.CSELU(x.imag))
...
class ComplexDropout(Module):
    ...
    def forward(self, input):
        if self.training:
            return complex_dropout(input, self.p,device=self.device)
        else:
            return input
