...
def complex_dropout(input, p=0.5, training=True,device = 'cpu'):
    # need to have the same dropout mask for real and imaginary part,
    # this not a clean solution!
    mask = torch.ones_like(input.real).type(torch.float32)
    # mask = torch.ones(*input.shape, dtype=torch.float32)
    mask = dropout(mask, p, training) * 1 / (1 - p)
    # mask = mask.clone().detach().to(device)
    return torch.complex((mask * input.real), (mask * input.imag))
...
def zReLU(input):
    real = relu(input.real)
    imag = relu(input.imag)

    real_flag = real.gt(0).float()
    imag_flag = imag.gt(0).float()
